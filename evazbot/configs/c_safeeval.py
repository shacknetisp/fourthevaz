...
class BadCompilingInput(Exception):
    ...
    """ The user tried to input something """ +\
    """which might cause compiler to slow down. """
...
def timeout(seconds, force_kill=True):
    ...
    Courtesy of http://code.activestate.com/recipes/
    577853-timeout-decorator-with-multiprocessing/
    """
...
def disassemble(co):
    """ Loop through Python bytecode and match instructions
    with our internal opcodes.
    ...
    while i < n:
        ...
        opv = globals()[opcode.opname[op].replace('+', '_')](co, curi,
                i, op, oparg)
...
class BINARY_POWER(BINARY):
    ...
    def touch_value(self, stack, frame):
        (TOS1, TOS) = stack[-2:]
        print (TOS1, TOS)
        if abs(TOS1) > BadValue.MAX_ALLOWED_VALUE or abs(TOS) \
            > BadValue.MAX_ALLOWED_VALUE:
...
def _safe_eval(expr, functions_and_constants={},
               check_compiling_input=True):
...
@timeout(0.1)
def safe_eval_timeout(expr, functions_and_constants={},
                      check_compiling_input=True):
    ...
    return _safe_eval(expr, functions_and_constants,
                      check_compiling_input)
