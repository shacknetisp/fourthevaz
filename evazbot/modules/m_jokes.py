    ...
    if mp.wcmd("addjoke"):
        ...
        if len(joke) < 1:
            ...
            with open(dbfile, "a") as myfile:
                myfile.write(joke + "\n")
                main.sendcmsg("Joke added to category '" + category + "': " + joke)
        return True
    if mp.cmd("joke"):
        target = ""
        try:
          ...
          main.sendcmsg(str(e)+" | Try enclosing <target> with \"quotes\"")
          return True
        except:
          traceback.print_exc()
        category = mp.argstr("c", "joke")
        ...
        if len(lines) > 0:
            ...
            if target:
                out = "#caller# tried to play a joke on #target#, but failed."
            else:
            ...
            for fname in files:
                ...
                for line in open(os.path.join(subdir, fname), 'r'):
                    if line.find("#target#") != -1:
                        targetcount += 1
                    totalcount += 1
                info = str(targetcount) + "/" + str(totalcount) + " #target#"
                jokecats.append(fname.replace(".db.txt", "") + ": " + info)
...
def showhelp():
    main.sendcmsg(
        ...
        "with the target being <target>, and the dbfile being <category>.")
    main.sendcmsg(
        ".addjoke [-c=<category>] <joke>: Add <the joke> to <category>. " +
        "#target# will be replaced with the target of .joke, #caller# will be replaced with the caller of the joke.")
    main.sendcmsg(".sortjokes: Clean the DB.")
    main.sendcmsg(".jokecats: List all categories, with number of #target# and regular lines.")
